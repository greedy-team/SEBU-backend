...
전공 계열 선택 PR #72는 담당 변경으로 병합하지 않는다. 이 크롤러는 계열 선택 코드나 컬럼에 의존하지 않는다.
처음에는 #72/#73의 V40/V41과 번호 충돌을 피하기 위해 V42를 사용했으며, SQL과 버전은 그대로 유지한다.
V40은 미사용으로 남긴다. Flyway는 버전의 연속성을 요구하지 않으므로 빈 V40 파일을 추가하지 않는다.

**계열 선택 코드를 분리한 졸업생 PR #73(V41)을 먼저 병합하고, 이후 이 PR(V42)을 병합한다.**
배포는 V41을 먼저 적용하거나, 두 PR이 포함된 빌드에서 V39 → V41 → V42 순서로 적용한다.
병합 직전 최신 `develop`을 반영하고 빈 DB 전체 적용 및 기존 DB 업그레이드를 MySQL에서 검증한다.
V42를 먼저 적용한 DB에 V41을 뒤늦게 넣으면 Flyway 순서 문제가 생길 수 있다.
다른 담당자가 계열 기능을 구현할 때는 V40을 재사용하지 않고 최신 통합·배포 버전 다음 번호를 사용한다.
V42까지 반영된 시점에 다른 예약 번호가 없다면 V43부터 추가한다.

적용 전 대상 DB의 `flyway_schema_history`를 확인한다. V40이 이미 적용됐거나 V42가 V41보다 먼저
적용된 DB가 있다면 기존 파일과 이력을 보존하고, 해당 이력에 맞는 후속 마이그레이션을 별도로 정한다.
